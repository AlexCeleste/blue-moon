...
Type TMetaParser Extends TParser Abstract
	...
	Global pp:PMeta
	Method Parse:TParseNode(toks:TToken[], mustComplete:Int = False)
		If Not _linked Then Link()
		Return Super.Parse(toks, mustComplete)
	End Method
	Method Link()
...
Type TParseNode
	Field rule:String, term:TToken
	Field name:String[], elem:TParseNode[]
	Method ToString:String()
		Function show:String(n:TParseNode, pad:Int)
			If n.term = Null
				If n.elem = Null Then Return "[NIL]~n"
			Else
				Return "{ '" + n.term.value + "' : " + n.term.tType + " }~n"
			EndIf
			
			Local id:String = LSet("", pad), s:String = n.rule + "~n"
			For Local e:Int = 0 Until n.elem.Length
				s :+ id + n.name[e] + ": " + show(n.elem[e], pad + 2)
			Next
			Return s
		End Function
		Return "root: " + show(Self, 2)
	End Method
	Method GetElem:TParseNode(n:String)
		For Local e:Int = 0 Until elem.Length
			If name[e] = n Then Return elem[e]
		Next
		Return Null
	End Method
	...
	End Function
	Function Node:TParseNode(el:TParseNode[], nm:String[])
		Local n:TParseNode = New Self ; n.elem = el ; n.name = nm ; Return n
	End Function
	Function Nil:TParseNode()
		Local n:TParseNode = New Self ; n.rule = "#Nil" ; Return n
	End Function
End Type
...
Type TParser Abstract
	Method Parse:TParseNode(toks:TToken[], mustComplete:Int = False)
		...
		If mustComplete And ct < toks.Length Then Throw ParseError.Make(toks[ct], "unexpected token '" + toks[ct].value + "' at document toplevel")
		Return ret
	End Method
...
Type TParseRule Abstract
	Field r:String, args:TParseRule[], _f:String[], noFold:Int
...
Type CatRule Extends TParseRule
	...
	Method filter:TParseNode(el:TParseNode[], p:TParser)
		Local ls:TList = CreateList(), name:String[] = New String[el.Length], ct:Int = 0, storedNil:Int = False
		If _f
			...
			For Local e:Int = 0 Until el.Length
				If el[e] <> OptRule.Nil
					If storedNil And _f[e][0] <> "-"[0]
						ls.RemoveLast() ; ct = 0 ; storedNil = False
					EndIf
					If _f[e][0] = "@"[0]
						If _f[e].Length > 1 Then name[ct] = _f[e][1..] Else name[ct] = "@" + ct
						ls.AddLast el[e] ; ct :+ 1
					ElseIf _f[e][0] = "<"[0]
						Local prev:TParseNode = el[e - 1]
						If prev <> OptRule.Nil
							If prev.elem
								prev.name :+ ["@" + prev.elem.Length]
								prev.elem :+ [el[e]]
							Else
								prev = TParseNode.Node([prev, el[e]], ["@0", "@1"])
								ls.RemoveLast() ; ls.AddLast prev
							EndIf
						Else
							prev = TParseNode.Node([el[e]], ["@0"])
							If _f[e - 1].Length > 1 Then name[ct] = _f[e - 1][1..] Else name[ct] = "@" + ct
							ls.AddLast prev ; ct :+ 1
						EndIf
					EndIf
				ElseIf ct = 0 And _f[e][0] = "@"[0]
					If _f[e].Length > 1 Then name[0] = _f[e][1..] Else name[0] = "@0"
					ls.AddLast OptRule.Nil ; ct = 1 ; storedNil = True
				EndIf
			Next
		Else
			For Local e:Int = 0 Until el.Length
		...
		If (el.Length > 1) Or noFold Then Return TParseNode.Node(el, name) Else Return el[0]
	End Method
	Method f:TParseRule(filt:String)
		_f = filt.Split(" ")
		If _f[_f.Length - 1] <> "^"
...
Type RepRule Extends TParseRule
	...
	Function RunF:TParseNode(s:TParseRule, p:TParser)
		...
		If ls = Null
			Return OptRule.Nil
		Else
			Local ret:TParseNode[] = TParseNode[](ls.ToArray()), name:String[] = New String[ret.Length]
			For Local i:Int = 0 Until ret.Length
				name[i] = "@" + i
			Next
			Return TParseNode.Node(ret, name)
		EndIf
	End Function
End Type
...
Type NamedRule Extends TParseRule
	Field impl:TParseRule = Null
	Method run:TParseNode(p:TParser)
		If impl = Null Then impl = p.GetNamedRule(r)
		Local ret:TParseNode = impl.run(p)
...
Type TLexer
	Field rules:TLexRule[], keyedRules:TLexRule[][], unkeyedRules:TLexRule[]
	Field cFile:LexFile
	...
	Function withRules:TLexer(r:TLexRule[])
		...
		l.rules = r ; l.keyedRules = New TLexRule[][128]
		For Local r:TLexRule = EachIn l.rules
			If r.iclass <> ""
				For Local c:Int = 0 Until r.iclass.Length
					l.keyedRules[r.iclass[c]] :+ [r]
				Next
			Else
				Local c:Int = r.pattern[0]
				If (c < 128) And ("()\[]^$.+*?|".Contains(Chr(c)) = False)
					l.keyedRules[c] :+ [r]
				Else
					l.unkeyedRules :+ [r]
				EndIf
...
Type TLexRule
	Field rule:TRegEx, pattern:String, iclass:String
	Field action(l:TLexer), result:String, mode:String
	Function Create:TLexRule(rs:String, act(l:TLexer), result:String = "", mode:String = "", iclass:String = "")
		Local r:TLexRule = New TLexRule
		r.rule = TRegEx.Create("\G" + rs, Null) ; r.pattern = rs ; r.iclass = iclass
		If result = "" And act = TLexAction.Store Then result = rs
		r.action = act ; r.result = result ; r.mode = mode
		Return r
	End Function
	Function CreateSimple:TLexRule(rs:String, act(l:TLexer), result:String = "", mode:String = "")	'non-regex - faster
		Local r:TLexRule = New TLexRule
		r.rule = Null ; r.pattern = rs
		If result = "" And act = TLexAction.Store Then result = rs
		r.action = act ; r.result = result ; r.mode = mode
		If rs[0] < 128 Then r.iclass = Chr(rs[0])
		Return r
	End Function
End Type
...
Function ScanLFile:TList(l:TLexer, f:LexFile)
	...
	l.out = CreateList()
	Repeat
		For Local r:TLexRule = EachIn l.rules	'Preload regexes with string data (wouldn't need this if we were using it normally)
			If r.rule Then r.rule.Find(l.cFile.stream, 0)
		Next
		
		While l.cFile.cPtr < l.cFile.sLen
			Local cf:LexFile = l.cFile, rule:TLexRule = Null, resLen:Int = 0
			
			Local ch:Int = cf.stream[cf.cPtr]
			If ch < 128
				Function tryRule(r:TLexRule, cf:LexFile, rule:TLexRule Var, resLen:Int Var)
					If r.rule	'regex
						Local reg:TRegEx = r.rule
						Local result:Int = pcre_exec(reg.pcre, Null, reg.targ, reg.targLength, cf.cPtr, reg.getExecOpt(), reg.offsets, reg.sizeOffsets)
						If result
							Local matchLen:Int = Int Ptr(reg.offsets)[1] - cf.cPtr
							If matchLen > resLen Then rule = r ; resLen = matchLen
						EndIf
					ElseIf r.pattern.Length > resLen	'simple
						Local ok:Int = True
						For Local c:Int = 0 Until r.pattern.Length
							If r.pattern[c] <> cf.stream[c + cf.cPtr] Then ok = False ; Exit
						Next
						If ok Then rule = r ; resLen = r.pattern.Length
					EndIf
				End Function
				For Local r:TLexRule = EachIn l.keyedRules[ch]
					tryRule r, cf, rule, resLen
				Next
				For Local r:TLexRule = EachIn l.unkeyedRules
					tryRule r, cf, rule, resLen
				Next
			EndIf
			
			If rule		'Something matched successfully!
				l.matchR = rule ; l.matchS = cf.stream[cf.cPtr..cf.cPtr + resLen]
				rule.action(l)
				cf.Increment resLen
			Else
				cf.Increment 1
